...
class LinchpinCli(LinchpinAPI):
    ...
    def find_topology(self, topology, lpf):
        ...
        topos = self.lp_topo_list(lpf["topology_upstream"])
        topos = [x["name"] for x in topos]
        if topology in topos:
            print("Found!!")
            print("Fetching topology from upstream !")
            self.lp_topo_get(topology, lpf["topology_upstream"])
            return os.path.abspath("./topologies/"+topology)
        print("Not Found !!")
        return None

    def find_layout(self, layout, lpf):
        config = self.get_config()
        try:
            ...
            layouts = self.lp_layout_list(lpf["layout_upstream"])
            layouts = [x["name"] for x in layouts]
            if layout in layouts:
                print("Found!!")
                print("Fetching layout from upstream !")
                self.lp_topo_get(layout, lpf["layout_upstream"])
        ...
        if target == "all":
            for key in set(lpf.keys()).difference(self.excludes):
                ...
                if e_vars['topology'] is None:
                    ...
                    break
                layout_path = self.find_layout(lpf[key]["layout"], lpf)
            ...
            if lpf.get(target, False):
                ...
                if e_vars['topology'] is None:
                    print("Topology not found !!")
                layout_path = self.find_layout(lpf[target]["layout"], lpf)
                e_vars['inventory_layout_file'] = layout_path
                if e_vars['inventory_layout_file'] is None:
                    print("Layout not found !!")
                print(e_vars)
