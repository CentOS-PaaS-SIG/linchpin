...
class ActionModule(ActionBase):
    def run(self, tmp=None, task_vars=None):
        ...
        print(task_name)
        if linchpin_mock:
            if task_name.startswith("libvirt_:_AAS"):
                return mock_utils.get_mock_data(module_args,
                                                "command_aas")
            if task_name.startswith("libvirt_:_MKISOFS"):
                return mock_utils.get_mock_data(module_args,
                                                "command_mkisofs")
            if task_name.startswith("libvirt_:_RCDR"):
                return mock_utils.get_mock_data(module_args,
                                                "command_remove_cd")
            if task_name.startswith("libvirt_:_GXMLDOV"):
                return mock_utils.get_mock_data(module_args,
                                                "command_gxmldov")
